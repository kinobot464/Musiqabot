...
BOT_TOKEN = "7780144299:AAEiGYayucjHGXMCxN0FPwDgjz7A-mTprko" ADMIN_ID = 2097478310 CHANNEL_USERNAME = "@AFSUNGAR_MERLIN_SERIALI_K" music_results = {}
...
def download_selected_music(url): ydl_opts = { 'format': 'bestaudio/best', 'outtmpl': 'music.%(ext)s', 'postprocessors': [{ 'key': 'FFmpegExtractAudio', 'preferredcodec': 'mp3', 'preferredquality': '192', }], 'quiet': True } with YoutubeDL(ydl_opts) as ydl: ydl.download([url])

def download_video(url): ydl_opts = { 'format': 'best[ext=mp4]', 'outtmpl': 'video.%(ext)s', 'quiet': True } with YoutubeDL(ydl_opts) as ydl: ydl.download([url])

def extract_audio(): subprocess.run(["ffmpeg", "-i", "video.mp4", "-vn", "-acodec", "libmp3lame", "-y", "audio.mp3"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

--- Telegram event handlerlari ---

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.effective_user.id register_user(user_id)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE): query = update.callback_query await query.answer() user_id = query.from_user.id data = query.data
